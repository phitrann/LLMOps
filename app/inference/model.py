...
class LLMModel:
    ...
    def _initialize_backend(self):
        if self.backend_type == BackendType.LLAMA_CPP:
            ...
            if torch.cuda.is_available():
                ...
                print("GPU CUDA not found. Running CPU backend with Torch Transformers.")
    ...
    def _create_model(self):
        if self.backend_type == BackendType.LLAMA_CPP:
            ...
            return Llama(model_path=self.model_path, n_ctx=self.max_tokens, n_batch=self.max_tokens, verbose=self.verbose)
        elif self.backend_type == BackendType.OLLAMA:
    ...
    async def generate(self, prompt: str, max_new_tokens: int = 1000, temperature: float = 0.9,
                 top_p: float = 1.0, top_k: int = 40, do_sample: bool = True, repetition_penalty: float = 1.0,
                 **kwargs: Any) -> AsyncIterator[str]:
        
        # Process the prompt, removing the kwargs used for prompt formatting
        prompt_kwargs = {k: v for k, v in kwargs.items() if k in ['chat_history', 'context', 'question']}
        ...
        if self.backend_type == BackendType.LLAMA_CPP:
            for chunk in self.model(processed_prompt, max_tokens=max_new_tokens, temperature=temperature,
            ...
            if self.verbose: print("Generator created")

            if self.verbose: print("Running generation loop ...")
            if self.timings:
                first = True
                new_tokens = []
            while not generator.is_done():
                ...
                if self.timings: new_tokens.append(new_token)
        else:  # Transformers
            ...
            await loop.run_in_executor(None, self._generate, generation_kwargs, streamer)
            
            async for text in self._async_stream(streamer):
                yield text

    def _generate(self, generation_kwargs, streamer):
        thread = Thread(target=self.model.generate, kwargs=generation_kwargs)
        thread.start()
        thread.join()

    async def _async_stream(self, streamer):
        while True:
            try:
                new_text = streamer.__next__()
                yield new_text
            except StopIteration:
                break

    def run(self, message: str, chat_history: list[tuple[str, str]] = [], system_prompt: str = "",
            max_new_tokens: int = 1000, temperature: float = 0.9, top_p: float = 1.0,
            top_k: int = 40, repetition_penalty: float = 1.0) -> Iterator[str]:
        prompt = get_prompt(message, chat_history, system_prompt)
        return self.generate(prompt, max_new_tokens, temperature, top_p, top_k, repetition_penalty)

    def _process_prompt(self, prompt: Union[str, StringPromptTemplate], **kwargs) -> str:
        if isinstance(prompt, StringPromptTemplate):
            return prompt.format(**kwargs)
        return prompt

    async def __call__(self, prompt: Union[str, StringPromptTemplate], stream: bool = False, max_new_tokens: int = 1000,
                       temperature: float = 0.9, top_p: float = 1.0, top_k: int = 40, do_sample: bool = True,
                       repetition_penalty: float = 1.0, **kwargs: Any) -> Union[str, Iterator[str]]:
        ...
        if self.backend_type == BackendType.LLAMA_CPP:
            return await self._generate_llama_cpp(processed_prompt, stream, max_new_tokens, temperature, top_p, top_k, repetition_penalty, **kwargs)
        elif self.backend_type == BackendType.OLLAMA:
            ...
            return await self._generate_transformers(processed_prompt, stream, max_new_tokens, temperature, top_p, top_k, do_sample, repetition_penalty, **kwargs)

    async def _generate_llama_cpp(self, prompt, stream, max_new_tokens, temperature, top_p, top_k, repetition_penalty, **kwargs):
    ...
    async def _generate_transformers(self, prompt, stream, max_new_tokens, temperature, top_p, top_k, do_sample, repetition_penalty, **kwargs):
    ...
    async def generate_text(self, prompt: str, streamer: TextIteratorStreamer):
        if self.backend_type == BackendType.ONNX:
            ...
            self.model.generate(input_ids, streamer=streamer, max_length=self.max_new_tokens)
